class ViewMatchs:
    ...
    def indicate_results(joueur1, joueur2, couleur_joueur1, couleur_joueur2):
        ...
        resultatJ1 = input(
            "Résultat de " + joueur1["prenom"] + " " + joueur1["nom_de_famille"] + " "
        )
        if float(resultatJ1) > 1 or float(resultatJ1) < 0:
            print(
                " Veuillez rentrer 1 pour une victoire , 0 pour une défaite ou 0.5 pour une égalité. "
            )
        resultatJ1 = float(resultatJ1)
        resultatJ2 = 1 - resultatJ1
        print(
            f"Les résultats du match sont: {resultatJ1} : {joueur1['prenom']} et {resultatJ2} : {joueur2['prenom']}"
        )
    ...
    def question_start_stop(n_tour_actualy):
        ...
        question = input(
            f" Voulez-vous commencer le tour {n_tour_actualy} [ o pour oui ]  ou quitter le programme maintenant [ N "
            f"pour non ]? 🚦 [o/N] "
        )
